% This algorithm is adapted from Too, et al. 
% "A New Quadratic Binary Harris Hawk Optimization for Feature Selection." 
% Electronics, vol. 8, no. 10, MDPI AG, Oct. 2019, p. 1130, doi:10.3390/electronics8101130.

function HHO = HarrisHawksOptimization_tstf(feat,label,opts)
  ...
  for i = 1:N
    ...
    elseif abs(E) < 1
      ...
      elseif r < 0.5 && abs(E) >= 0.5
        ...
        Ty = taper_tf(Y,2,1); % T1
        By = double(rand(size(Y)) < Ty);
        Tz = taper_tf(Z,2,1); % T1
        Bz = double(rand(size(Z)) < Tz);

        % Fitness
        fitY = fun(feat,label,By,HO);
        fitZ = fun(feat,label,Bz,HO);
        % Greedy selection (10)
        if fitY < fit(i)
          fit(i) = fitY; 
          X(i,:) = Y;
